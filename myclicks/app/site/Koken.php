...
META;
	...
	public static function breadcrumbs($options = array())
	{
		...
		if (isset($single['context']['type']))
		{
			...
			else if ($single['context']['type'] !== 'favorite' && $single['context']['type'] !== 'feature' && isset(self::$site['urls'][$single['context']['type']]))
			{
	...
	private static function callback($matches)
	{
		...
		else
		{
			...
			else if (in_array($action, array('content', 'essays', 'albums', 'categories', 'topics')))
			{
		...
		if (strpos($key, '|') === false)
		{
			...
			else
			{
				while(count($keys))
				{
					...
					else if ((!is_array($return) || !isset($return[$index])) && $index === 'clean' && !isset($return['raw']))
					{
					...
					else if (isset($global_key) && $global_key === 'settings' && isset(Koken::$settings['__scoped_' . str_replace('.', '-', Koken::$location['template']) . '_' . $index]))
					{
	...
	public static function link($parameters)
	{
		...
		if ($options['share'])
		{
			...
			else if (isset($token['album']) && count($token['album']['covers']))
			{
				$image = $token['album']['covers'][0]['presets']['large']['url'];
			}
			else if (isset($token['featured_image']) && $token['featured_image'])
			{
			...
			{
				if (strpos($key, 'filter:') === 0) continue;
	...
	public static function form_link($obj, $ctx, $lightbox)
	{
		...
		else
		{
			...
			else if (!isset($obj['__koken__']) && isset($obj['items']) && isset($obj['year']))
			{
				$type = 'event_timeline';
			}
			else
	...
	public static function context_parameters($type = 'content')
	{
		if ($type === 'content')
		{
			...
			else
			{
				...
				else if ($featured_regex && preg_match('~' . $featured_regex . '~', self::$location['here']))
				{
	...
	static public function load($params)
	{
		...
		else if ($source['type'] === 'tag' && isset($defaults['api']['tag']))
		{
...
OUT;
	...
	static function sort_tags_by_count($a, $b)
	{
	...
	static function sort_tags_by_count_asc($a, $b)
	{
		if ($a['counts']['total'] < $b['counts']['total'] || ($a['counts']['total'] === $b['counts']['total'] && $a['title'] < $b['title']))
		{
			return -1;
